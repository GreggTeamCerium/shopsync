...
export function ProductsTable({ products }: ProductsTableProps) {
  ...
  function handleMarkSold(id: string) {
    ...
    startTransition(async () => {
      const res = await markProductSoldAction(id);
      if (!res.ok) {
        setActionError(res.error ?? "Failed to mark as sold");
        return;
      }
      setConfirmSold(null);
      router.refresh();
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      {confirmSold && (
        <ConfirmDialog
          title="Mark as Sold"
          message={`Mark "${products.find((p) => p.id === confirmSold)?.name}" as sold?`}
          confirmLabel={isPending ? "Saving..." : "Mark as Sold"}
